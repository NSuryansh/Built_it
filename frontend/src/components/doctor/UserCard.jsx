...
const UserCard = ({ userWithAppointments }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-[var(--custom-white)] to-[var(--custom-blue-50)]/50 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm hover:shadow-md transition-all duration-300 border border-[var(--custom-blue-100)]/50 group transform hover:-translate-y-1">
      <div className="grid grid-cols-1 md:grid-cols-[1fr_3fr_auto] gap-4 sm:gap-6">
        <div className="space-y-3">
          <div className="space-y-1 text-center md:text-left">
            <h3 className="font-bold text-lg text-[var(--custom-blue-900)]">
              {user.user.username}
            </h3>
            <p className="text-sm text-[var(--custom-gray-600)] flex items-center justify-center md:justify-start">
              <UserIcon className="h-3.5 w-3.5 mr-1 text-[var(--custom-blue-400)]" />
              {user.user.email}
            </p>
          </div>
        </div>
