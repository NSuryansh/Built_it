...
const ErrorBoundaryFallback = ({ userType }) => {
  ...
  const bgClass = `bg-custom-${color}-100`;
  const iconColorClass = `text-custom-${color}-500`;
  const buttonBgClass = `bg-custom-${color}-500 hover:bg-custom-${color}-600`;
        ...
        <div className="flex flex-col items-center text-center">
          <AlertOctagon
            className={`w-16 h-16 ${iconColorClass} ${
              userType === "admin" ? "text-custom-green-700" : ""
            } mb-4`}
          />
          ...
          <button
            ...
            className={`inline-flex items-center px-4 py-2 ${buttonBgClass} text-custom-white rounded-lg transition-colors`}
          >
