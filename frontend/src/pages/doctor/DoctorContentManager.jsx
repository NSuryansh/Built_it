...
const DoctorContentManager = () => {
  const [activeTab, setActiveTab] = useState("articles"); // articles, videos, entertainment
  ...
  useEffect(() => {
    const verifyAuth = async () => {
      ...
      if (!authStatus) {
        ...
        setDoctorToken(localStorage.getItem("token"));
        fetchData(activeTab); // Load data initially
      }
    };
    verifyAuth();
  }, [navigate]);
  ...
  const fetchData = async (tab) => {
    ...
    try {
      ...
      else if (tab === "videos") endpoint = "/get-videos"; // Note: Returns grouped data, need flat list for table?
      else if (tab === "entertainment") endpoint = "/get-entertainment";

      const res = await axios.get(`${backendUrl}${endpoint}`);
      
      // Flatten video data if needed (since get-videos returns grouped)
      if (tab === "videos") {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            ...
            <div className="bg-white p-1 rounded-lg shadow-sm border border-gray-200 flex">
                <button 
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${viewMode === "list" ? "bg-[var(--custom-orange-100)] text-[var(--custom-orange-700)]" : "text-gray-600 hover:bg-gray-50"}`}
                >
                    Manage List
                </button>
                <button 
                    onClick={() => { setViewMode("form"); resetForms(); }}
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${viewMode === "form" ? "bg-[var(--custom-orange-600)] text-white" : "text-gray-600 hover:bg-gray-50"}`}
                >
        ...
        {/* Status Message */}
        {message.text && (
          <div className={`mb-6 p-4 rounded-lg flex items-center gap-3 animate-fade-in ${message.type === "success" ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}>
        ...
        {viewMode === "form" && (
            <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-100 max-w-3xl mx-auto">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                    ...
                    <button onClick={() => { setViewMode("list"); resetForms(); }} className="text-gray-400 hover:text-gray-600">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                        ...
                        <>
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:bg-gray-50 transition-colors">
                                ...
                                <label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center">
                                    {entForm.currentImageUrl && !entForm.image && (
                                        <img src={entForm.currentImageUrl} alt="Current" className="h-20 w-auto mb-2 rounded shadow-sm object-cover" />
                                    )}
                                    <div className="p-3 bg-gray-100 rounded-full mb-2 text-gray-500">
                                        <Upload className="w-5 h-5" />
                                    </div>
                                    <span className="text-sm font-medium text-gray-900">
                                        {entForm.image ? entForm.image.name : (editMode ? "Change Cover Image (Optional)" : "Upload Cover Image")}
                                    </span>
                                </label>
                            </div>
                        </>
