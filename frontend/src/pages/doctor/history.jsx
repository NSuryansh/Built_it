...
const History = () => {
  ...
  const getPastAppointments = async () => {
    try {
      ...
      const response = await fetch(
        `https://built-it.onrender.com/api/doc/pastdocappt?doctorId=${docId}`,
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-[var(--custom-blue-50)] via-[var(--custom-white)] to-[var(--custom-teal-50)]">
      <ToastContainer />
      <DoctorNavbar />

      <main className="max-w-7xl xl:min-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        <div className="bg-[var(--custom-white)]/80 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-xl border border-[var(--custom-blue-100)]/50 overflow-hidden">
          <div className="p-4 sm:p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-4 sm:space-y-0 sm:space-x-4">
              <div className="flex items-center space-x-3">
                <div className="p-2 bg-[var(--custom-blue-100)] rounded-xl">
              ...
              <div className="relative w-full sm:w-64 md:w-96">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Search className="h-5 w-5 text-[var(--custom-gray-400)]" />
                </div>
                <input
                  type="text"
