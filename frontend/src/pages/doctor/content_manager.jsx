...
const DoctorContentManager = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `Item ${editMode ? "updated" : "added"} successfully!`,
      });
      resetForms();
      fetchData(activeTab);
      setViewMode("list");
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-600">
              Manage Stress & Entertainment Content
            </p>
          </div>
          <div className="bg-white p-1 rounded-lg shadow-sm border border-gray-200 flex">
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${viewMode === "list" ? "bg-[var(--custom-blue-100)] text-[var(--custom-blue-700)]" : "text-gray-600 hover:bg-gray-50"}`}
            >
              Manage List
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${viewMode === "form" ? "bg-[var(--custom-blue-600)] text-white" : "text-gray-600 hover:bg-gray-50"}`}
            >
        ...
        {message.text && (
          <div
            className={`mb-6 p-4 rounded-lg flex items-center gap-3 ${message.type === "success" ? "bg-green-50 text-green-700 border border-green-200" : "bg-blue-50 text-blue-700 border border-blue-200"}`}
          >
            ...
            {fetching ? (
              ...
              <div className="p-12 text-center text-gray-500">
                ...
                <button
                  ...
                  className="mt-4 text-[var(--custom-blue-600)] font-medium hover:underline"
                >
                        ...
                        <td className="p-4 text-right">
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              ...
                              className="p-2 text-red-600 hover:bg-blue-50 rounded-lg"
                            >
        ...
        {viewMode === "form" && (
          <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-100 max-w-3xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <SelectField
                      ...
                      options={["Book", "Sun", "Brain", "Heart", "Headphones"]}
                    />
                  </div>
                  <InputField
                    label="URL"
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <SelectField
                      ...
                      options={["Meditation", "Yoga", "Breathing", "Sleep"]}
                    />
                    <InputField
                      label="Title"
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <input
                        list="type-options"
                        type="text"
                        className="w-full p-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[var(--custom-blue-500)] outline-none"
                        value={entForm.type}
                        onChange={(e) =>
                          setEntForm({ ...entForm, type: e.target.value })
                        }
                    ...
                    <InputField
                      label="Category"
                      value={entForm.category}
                      onChange={(e) =>
                        setEntForm({ ...entForm, category: e.target.value })
                      }
                      placeholder="e.g. Top Picks, Action"
                    />
                    <InputField
                      label="Title"
                      value={entForm.title}
                      onChange={(e) =>
                        setEntForm({ ...entForm, title: e.target.value })
                      }
                    />
                    <InputField
                      label="Link"
                      value={entForm.link}
                      onChange={(e) =>
                        setEntForm({ ...entForm, link: e.target.value })
                      }
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:bg-gray-50 transition-colors">
                    <input
                      ...
                      accept="image/*"
                      onChange={(e) =>
                        setEntForm({ ...entForm, image: e.target.files[0] })
                      }
                    />
                    <label
                      htmlFor="file-upload"
                    ...
                    >
                      {getPreviewUrl() && (
                        <img
                          src={getPreviewUrl()}
                          alt="Preview"
                          className="h-40 w-auto mb-4 rounded-lg shadow-md object-cover"
                        />
                      )}
                      <div className="p-3 bg-gray-100 rounded-full mb-2 text-gray-500">
              ...
              <SubmitButton
                isLoading={isLoading}
                label={editMode ? "Update Item" : "Create Item"}
              />
            </form>
          </div>
        )}
...
const TabButton = ({ active, onClick, icon: Icon, label }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-2 px-6 py-4 border-b-2 text-sm font-medium transition-all ${active ? "border-[var(--custom-blue-500)] text-[var(--custom-blue-600)] bg-[var(--custom-blue-50)]" : "border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50"}`}
  >
...
const InputField = ({ label, value, onChange, placeholder }) => (
  <div>
    ...
    <input
      type="text"
      className="w-full p-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[var(--custom-blue-500)] focus:border-transparent outline-none transition-all"
      value={value}
      onChange={onChange}
...
const TextAreaField = ({ label, value, onChange }) => (
  <div>
    ...
    <textarea
      className="w-full p-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[var(--custom-blue-500)] outline-none min-h-[100px]"
      value={value}
      onChange={onChange}
      required
    />
...
const SubmitButton = ({ isLoading, label }) => (
  <button
    type="submit"
    disabled={isLoading}
    className="w-full bg-[var(--custom-blue-600)] hover:bg-[var(--custom-blue-700)] text-white font-bold py-4 rounded-xl shadow-lg transition-all transform hover:-translate-y-0.5 disabled:opacity-70 disabled:cursor-not-allowed flex justify-center items-center gap-2"
  >
    {isLoading ? (
