...
const DoctorPeer = () => {
  ...
  async function fetchContacts(userId) {
    try {
      const response = await fetch(
        `https://built-it.onrender.com/user_doc/chatContacts?userId=${userId}&userType=doc`,
  ...
  async function fetchMessages(userId, recipientId) {
    try {
      ...
      const response = await fetch(
        `https://built-it.onrender.com/user_doc/messages?userId=${recipientId}&recId=${userId}&userType=doc&recType=user`,
        { headers: { Authorization: "Bearer " + token } }
      );
